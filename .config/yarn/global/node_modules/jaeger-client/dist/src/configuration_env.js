...
var ConfigurationEnv = function () {
  ...
  _createClass(ConfigurationEnv, null, [{
    ...
    value: function _getReporterFromEnv(config) {
      ...
      value = ConfigurationEnv._getConfigValue(config.reporter, 'collectorEndpoint', process.env.JAEGER_REPORTER_ENDPOINT);
      if (value) {
      ...
      value = ConfigurationEnv._getConfigValue(config.reporter, 'password', process.env.JAEGER_REPORTER_PASSWORD);
      if (value) {
        reporterConfig.password = value;
      }

      value = ConfigurationEnv._getConfigValue(config.reporter, 'agentHost', process.env.JAEGER_REPORTER_AGENT_HOST);
      if (value) {
        reporterConfig.agentHost = value;
      }

      value = ConfigurationEnv._getConfigValue(config.reporter, 'agentPort', process.env.JAEGER_REPORTER_AGENT_PORT);
      if (value) {
